...
LoaderObject::LoaderObject(char*name,double R,double G,double B)
{
	this->R=R;
	this->G=G;
	this->B=B;
...
void LoaderObject::cargarObject(char*name,LoaderObject*l){
	fstream f;
	char c[100];
	f.open(name,ios::in);
	   if(f.is_open())
...
string LoaderObject::findd(string s,int caso){
...
void LoaderObject::recorrerArchiv(char*c,LoaderObject*l){
...
void LoaderObject::pintarse(LoaderObject*l){
...
void LoaderObject::pintarNewBoj(LoaderObject*l){
	//glColor3f((GLfloat)l->R,(GLfloat)l->G,(GLfloat)l->B);
...
void LoaderObject::Add(GLfloat*Vertices,LoaderObject*l){
	if(l->contv>=l->cantv)
	{
